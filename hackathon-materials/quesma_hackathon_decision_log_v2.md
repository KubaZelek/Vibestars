...
## 15. Stage 3 judge v1 smoke-test findings

The first semantic judge run contained 20 control segments because the v1 input was ordered by sample role.

Observed:
- 18/20: no plan mismatch
- 18/20: no stagnation
- 19/20: no thrashing
- mean estimated avoidable fraction: ~4%
- OpenRouter-reported cost: ~$1.85 for ~514k total tokens

These results are useful as a smoke test, but not as an unbiased risk-vs-control comparison.

Two controls contained credible issues, confirming an important principle:
**controls are comparative baselines, not negative labels.**

## 16. Methodological fixes after judge v1

### Sample-label leakage
The judge did not receive a `sample_role` field, but the v1 `segment_id` itself contained `control__` or `risk__`.

Fix:
use a neutral hashed `audit_id`. Original IDs and roles remain only in a sidecar never sent to the model.

### Ordered pilot
The first 20 v1 inputs were all controls.

Fix:
v2 is shuffled. The pilot contains 10 control + 10 risk, stratified across Claude-like, Codex and OpenCode.

### Task requirements vs agent plan
V1 sometimes produced Plan Fidelity scores even when `explicit_plan_present=false`.

Fix:
only agent-authored future actions count as an explicit plan.
If no explicit plan:
- `plan_obligations=[]`
- `plan_mismatch_class=not_applicable`
- `plan_fidelity_score=null`

Task completion is evaluated separately.

### Heuristic plan candidate
The parser's `plan_candidate` can occasionally be a retrospective final summary rather than an actual plan.

Fix:
do not send `plan_candidate` to the judge.
The judge extracts plans only from actual trace evidence.

## 17. Judge v2 additional outputs

V2 adds:
- task completion status and requirement evidence
- nullable Plan Fidelity
- semantic progress events
- state-aware stagnation and thrashing
- earliest safe intervention
- conservative avoidable-work estimate

Final comparative statistics should use only genuinely blinded v2 judgments.
